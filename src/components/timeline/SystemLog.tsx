...
type LogEntryType = (typeof LOG_ENTRIES)[number];
...
function TimelineRow({
  ...
  const handleClick = () => {
    if (!isCompact) return;
    setActiveYear(isActive ? null : entry.year);
  };

  const handleHoverStart = () => {
    if (!isCompact) setActiveYear(entry.year);
  };

  const handleHoverEnd = () => {
    if (!isCompact) setActiveYear(null);
  };
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4">
          <span
            className="font-mono text-[10px] sm:text-xs px-2 py-0.5 rounded shrink-0"
            style={{
          ...
          <span
            className="font-mono text-xs sm:text-sm font-bold shrink-0 w-10 sm:w-12"
...
export default function SystemLog() {
  ...
  return (
    <section
      id="timeline"
      ref={ref}
      className="snap-section min-h-screen flex items-center relative overflow-hidden bg-dark-950 py-12 md:py-20"
    >
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 w-full">
        ...
        >
          ...
          <p className="text-slate-600 mt-4 text-sm font-mono">
            {isCompact ? "tap entries to expand" : "hover entries to expand"}
          </p>
        </motion.div>
